...
export default function TabGroupUpdateForm(props) {
  const {
    id: idProp,
    tabGroup: tabGroupModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tabGroupRecord
      ? { ...initialValues, ...tabGroupRecord }
      : initialValues;
    setData(cleanValues.data);
    setEncrypted_with_password(cleanValues.encrypted_with_password);
    setErrors({});
  };
  const [tabGroupRecord, setTabGroupRecord] = React.useState(tabGroupModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(TabGroup, idProp)
        : tabGroupModelProp;
      setTabGroupRecord(record);
    };
    queryData();
  }, [idProp, tabGroupModelProp]);
  React.useEffect(resetStateValues, [tabGroupRecord]);
  const validations = {
    data: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            TabGroup.copyOf(tabGroupRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TabGroupUpdateForm")}
      {...rest}
    >
      <TextField
        label="Data"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tabGroupModelProp)}
